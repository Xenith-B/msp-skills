...
import (
	...
	"riverside-fm-pp-cli/internal/cliutil"
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "riverside-fm-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			// Cookie-imported sessions (auth login --chrome) store the full
			// "name=value; name=value" cookie string in Config.AccessToken with
			// AuthSource="browser". Riverside's cookie-authed API needs the
			// Cookie header, not Authorization. For all other auth sources,
			// keep the historical Authorization behavior.
			if c.Config != nil && c.Config.AuthSource == "browser" {
		...
		// Riverside sliding session: every response rotates a fresh riverside_auth_access
		// cookie (validated against the long-lived refresh cookie). Honor the rotation so
		// the stored cookie never goes stale, and retry once on 401/403 — a cold 403 still
		// carries a fresh access cookie, and the retry then succeeds.
		if c.Config != nil && c.Config.AuthSource == "browser" {
			if rotated := mergeRotatedCookies(c.Config.AccessToken, resp.Cookies()); rotated != c.Config.AccessToken {
				c.Config.AccessToken = rotated
				authHeader = rotated
				_ = c.Config.SaveAccessToken(rotated) // best-effort; keeps next invocation fresh
			}
			if (resp.StatusCode == 401 || resp.StatusCode == 403) && !authRetried {
				authRetried = true
				lastErr = nil
				continue
			}
...
func (c *Client) refreshAccessToken() error {
	...
	// Riverside has no separate refresh endpoint. It refreshes via a *sliding session*:
	// every API response (200, 403, even 404) carries a Set-Cookie that rotates a fresh
	// riverside_auth_access (~15-min) validated against the ~60-day refresh cookie. The
	// real "refresh" is therefore handled inline in do(): honor the rotated Set-Cookie on
	// every response (mergeRotatedCookies) and retry once on 401/403 with the fresh cookie.
	return nil
}

// mergeRotatedCookies folds freshly-rotated riverside_auth_* values from a response's
// Set-Cookie headers into the stored "name=value; name=value" cookie string. Cleared or
// non-JWT values are ignored so a logout/blank cookie can't clobber a working session.
func mergeRotatedCookies(cookieStr string, cookies []*http.Cookie) string {
	for _, ck := range cookies {
		if ck.Name != "riverside_auth_access" && ck.Name != "riverside_auth_refresh" {
			continue
		}
		if ck.Value == "" || ck.Value == "deleted" || strings.Count(ck.Value, ".") != 2 {
			continue
		}
		parts := strings.Split(cookieStr, "; ")
		found := false
		for i, p := range parts {
			if strings.HasPrefix(p, ck.Name+"=") {

...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".riverside-fm-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RIVERSIDE_FM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RIVERSIDE_FM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "riverside-fm-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.riverside-fm-cli/feedback.jsonl.
When ` + "`RIVERSIDE_FM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RIVERSIDE_FM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  riverside-fm-cli feedback list
  riverside-fm-cli feedback list --limit 5
  riverside-fm-cli feedback list --json`,

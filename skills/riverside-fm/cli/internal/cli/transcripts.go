...
func newTranscriptsConvertCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     "  riverside-fm-cli transcripts convert bf487406-af40-4bb4-b7f9-a6b49047b55d --format vtt --out ep.vtt",
...
func newTranscriptsTalktimeCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		...
		Example:     "  riverside-fm-cli transcripts talktime bf487406-af40-4bb4-b7f9-a6b49047b55d --json",
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()

...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  riverside-fm-cli doctor
  riverside-fm-cli doctor --json
  riverside-fm-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "riverside-fm-cli auth login --chrome"
				} else {
					authConfigured = true
					report["auth"] = "configured (browser session)"
					report["auth_source"] = cfg.AuthSource
					report["auth_domain"] = "riverside.com"
				}
			}
			// Check cookie tool availability. detectCookieTool probes the same
			// three backends auth login uses, each named as a compile-time
			// literal, so doctor's verdict cannot drift from what login does.
			if tool, err := detectCookieTool(); err == nil {
			...
			if cfg != nil && doctorBaseURLIsPlaceholder(cfg.BaseURL) {
				// Never dial a stand-in host. Several placeholder domains resolve
				// through live wildcard DNS and answer real HTTP, so probing one
				// produces a confident green verdict against a host the operator
				// has never heard of. Name the variable that fixes it.
				report["api"] = "ERROR not checked: base_url is a placeholder (" + cfg.BaseURL + "); set " + doctorBaseURLEnv
				report["credentials"] = "ERROR not verified: base_url is a placeholder, so nothing was probed; set " + doctorBaseURLEnv
				...
				if clientErr != nil {
					...
					//
					// Probe a REAL endpoint, not "/". A wrong base_url - the
					// vendor's web UI instead of its API - answers 200 at the
					// root and 404 everywhere else, so a root probe reports
					// "reachable" for an install that cannot make a single API
					// call. The path is derived from the command tree rather
					// than hardcoded, so it stays correct as the surface moves.
					healthPath, _ := doctorReadProbe(cmd.Root())
					if healthPath == "" {
						healthPath = "/"
					}
					...
					if authHeader == "" {
						...
						doctorProbeCredentials(c, cmd.Root(), "riverside-fm-cli", report)
					}
				}
			} else if cfg != nil && cfg.BaseURL == "" {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("riverside-fm-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'riverside-fm-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'riverside-fm-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'riverside-fm-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'riverside-fm-cli sync' to refresh."
	}
	return report
}

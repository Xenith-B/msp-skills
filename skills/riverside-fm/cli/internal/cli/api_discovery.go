...
Run 'api <interface>' to see that interface's methods.`,
  ...
  riverside-fm-cli api

  # Show methods for a specific interface
  riverside-fm-cli api <interface-name>`,
			...
			if len(args) > 0 {
				...
				for _, child := range root.Commands() {
					if child.Hidden && strings.ToLower(child.Name()) == target {
						...
						fmt.Fprintf(cmd.OutOrStdout(), "\nUse '%s-cli %s <method> --help' for details.\n", "riverside-fm", child.Name())
						return nil
					}
				}
				return fmt.Errorf("interface %q not found. Run '%s-cli api' to list all interfaces", args[0], "riverside-fm")
			}

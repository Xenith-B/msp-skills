...
Exit codes & warnings:
  ...
  riverside-fm-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  riverside-fm-cli sync --full

  # Incremental sync: only records from the last 7 days
  riverside-fm-cli sync --since 7d

  # Parallel sync with 8 workers
  riverside-fm-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  riverside-fm-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/riverside-fm-cli/data.db)")

...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "riverside-fm-cli",
		Short: `Riverside CLI with priority-fallback grab, bulk studio export, and cross-studio readiness for Pro-tier accounts`,
		Long: `Riverside CLI — The first programmatic way to download every transcript, audio track, and video from your own Riverside.com account — no Business plan or API key required.
...
Highlights (not in the official API docs):
  ...
  • bulk export   Walk every project / take / asset in a studio (or date range) and download transcripts + per-participant audio + HLS manifests with a resume cursor on '.runstate/'. The killer workflow no Pro-tier to…
...
Health check: run 'riverside-fm-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("riverside-fm-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'riverside-fm-cli profile list')")
	...
	clipsCmd := newClipsCmd(flags)
	clipsCmd.AddCommand(newClipsHarvestCmd(flags))
	rootCmd.AddCommand(clipsCmd)
	rootCmd.AddCommand(newProjectsCmd(flags))
	rootCmd.AddCommand(newStudiosCmd(flags))
	rootCmd.AddCommand(newTakesCmd(flags))
	// Riverside CLI novel features (Phase 3)
	...
	rootCmd.AddCommand(newWaitCmd(flags))
	...
	rootCmd.AddCommand(newProductionsPromotedCmd(flags))
	rootCmd.AddCommand(newRecordingsPromotedCmd(flags))
	rootCmd.AddCommand(newTranscriptionsPromotedCmd(flags))
	rootCmd.AddCommand(newUserPromotedCmd(flags))
	rootCmd.AddCommand(newVodPromotedCmd(flags))

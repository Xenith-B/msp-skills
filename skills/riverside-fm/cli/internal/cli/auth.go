...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Riverside",
	}
...
Use --chrome to read cookies from Chrome for riverside.com.
...
profile by name when the installed backend supports it.`,
		Example: `  riverside-fm-cli auth login --chrome
  riverside-fm-cli auth login --browser
  riverside-fm-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  riverside-fm-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  riverside-fm-cli auth login --browser\n")
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  riverside-fm-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  riverside-fm-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: riverside.com\n")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  riverside-fm-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func chromeDataDir() (string, error) {
	...
	case "windows":
		// os.UserCacheDir() IS %LocalAppData% on Windows, with the same
		// fall-back-to-the-profile behaviour, so use it rather than reading
		// LOCALAPPDATA directly: a raw os.Getenv of an OS variable reads to the
		// env-schema gate as an undeclared operator credential the .mcpb bundle
		// would have to prompt for, which it must never do.
		localAppData, cacheErr := os.UserCacheDir()
		if cacheErr != nil || localAppData == "" {
...
func readProfileDisplayName(prefsPath string) string {
	// #nosec G304 -- prefsPath is joined from the OS-standard Chrome data dir and a
	// directory entry listed from it; never caller input.
	data, err := os.ReadFile(filepath.Clean(prefsPath))
	if err != nil {
		return ""
	}
	var prefs struct {
...
func copyFileIfExists(src, dst string) error {
	// #nosec G304 -- src is the operator's own Chrome Cookies DB under their home dir.
...
// runPythonScript writes script to a private temp file and runs it under
// python3, returning stdout.
//
// The script is deliberately NOT passed via `python3 -c`. A `-c` payload is
// visible in the process table to every other account on the machine, and this
// script carries the absolute path of the operator's Chrome profile. Writing it
// 0600 inside a 0700 temp dir keeps it readable only by the operator, and the
// interpreter name stays a compile-time literal.
func runPythonScript(script string) ([]byte, error) {
	dir, err := os.MkdirTemp("", "riverside-cookie-")
	if err != nil {
		return nil, fmt.Errorf("creating temp dir: %w", err)
	}
	defer os.RemoveAll(dir)
	scriptPath := filepath.Join(dir, "extract.py")
	if err := os.WriteFile(scriptPath, []byte(script), 0o600); err != nil {
		return nil, fmt.Errorf("writing helper script: %w", err)
	}
...
//
// Every probe below names its executable as a compile-time literal. The complete
// set of programs this CLI can ever launch is fixed at build time - python3,
// cookies, cookie-scoop, agent-browser, browser-use - and no caller input selects
// or shapes the executable, only its documented arguments.
func detectCookieTool() (string, error) {
	if _, err := runPythonScript("import pycookiecheat"); err == nil {

...
- **`ready`**  -  List every take across all your synced studios that is fully ready: cloud backup done, transcription finished, no participant track still uploading.
  ...
  ```bash
  riverside-fm-cli ready --json --select studio,project,take_id,duration
  ```
- **`wait`**  -  Block until a take's backup, transcription, and/or AI generation are done; --include selects which facets to wait on; --timeout caps the wait. Exits 0 on ready, 2 on timeout.
  ...
  ```bash
  riverside-fm-cli wait <session-id> --include transcript,assets,ai --timeout 30m
  ```
...
Run `riverside-fm-cli --help` for the full command reference and flag list.
...
- **`riverside-fm-cli productions get_media`** - List media board items (sound effects, intros, jingles) for a production. Includes signed CloudFront URLs.
...
- **`riverside-fm-cli recordings get_backup_status`** - Get the cloud backup status for a single recording. Status values - none, processing, done.
...
- **`riverside-fm-cli transcriptions <session-id>`** - Get the editable transcript with voice activity for a take. Returns speakers, segments, timestamps. This is the raw data the CLI converts to TXT/SRT/VTT/JSON output.
...
- **`riverside-fm-cli vod <session-id> <participant-handle>`** - Get the HLS m3u8 manifest for a participant's video. Used to stream or transcode.
...
riverside-fm-cli clips get <clip-id>

# JSON for scripting and agents
riverside-fm-cli clips get <clip-id> --json

# Filter to specific fields
riverside-fm-cli clips get <clip-id> --json --select id,name,status

# Dry run  -  show the request without sending
riverside-fm-cli clips get <clip-id> --dry-run

# Agent mode  -  JSON + compact + no prompts in one flag
riverside-fm-cli clips get <clip-id> --agent
```
...
Install the skill from this repository  -  it ships the CLI this repo builds and
releases, which the printing-press library does not:

```bash
curl -fsSL https://raw.githubusercontent.com/Servosity/msp-skills/main/skills/riverside-fm/install.sh | sh
```

On Windows, run `install.ps1` from the same directory instead. Then invoke the
skill in Claude Code. The skill is the most efficient path  -  Claude Code drives
the CLI directly without an MCP server in the middle.
...
claude mcp add riverside-fm riverside-fm-mcp
```
...
Config file: `~/.config/riverside-fm-cli/config.toml`

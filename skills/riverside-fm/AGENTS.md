# Riverside Printed CLI Agent Guide

This directory is a generated `riverside-fm-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
riverside-fm-cli doctor --json
riverside-fm-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
riverside-fm-cli which "<capability>" --json
riverside-fm-cli <command> --help
```
...
```bash
riverside-fm-cli <command> --agent
```
...
```bash
riverside-fm-cli <command> --help
riverside-fm-cli <command> --dry-run --agent
```
